...
class objecttracker():
	...
	def Addto_trackerlist(self,frame,box,classname,retscore,trackingage):
		...
		if box[0]>0 and box[1]>0 and box[2]>0 and box[3]>0 and ymin >5 :
			try:
				...
				self.trackerid+=1
				print("tracker_list",trackerlist)
	...
	def gettrackinglabels(self,frame):
			...
			if ((w/width) <= 0.5 and (h/height) <= 0.5) or (xmin > 40 and ymin>40) or (xmin+w) < (width-10) and (ymin+h) < (height-10) :
				...
				if w>0 and h >0  and  xmin >0 and ymin > 0 and trackingage <= self.trackingagelimit :
					...
					if overlap==True:
						...
						print("ymin, xmin valuesss", ymin,xmin,ymax, xmax)
						#ymin=ymin*h
						#xmin=xmin*w
						#ymax=ymax*h
						#xmax=xmax*w		
		...
		print(boxes,classes,trackerids,retscores)
	...
	def update_trackingage(self):
		tracklistnew=[]	
		print("trackerlist-update", self.trackerlist)
		for tracker in self.trackerlist:
			tracker['trackingage']+=1
			tracklistnew.append(tracker)
		self.trackerlist=tracklistnew
		print("update_trackingageeeee", self.trackerlist)
	...
	def trackingobjects(self,frame,nboxes,nclasses,retscores):
		...
		tboxes,classes,trackerids,tretscores = self.gettrackinglabels(frame)
		print("tboxes and labels!!!!",tboxes,classes)
		# loop over the bounding boxes and draw then on the frame
		print("lengthhhhhhhhh",len(nboxes), len(tboxes), (len(tboxes) == 0))
		if len(nboxes) > 0:	
			print("nboxes>0")
			if len(tboxes) == 0:
				print("tboxes>0")
				print("valuess", nboxes,nclasses,retscores)
				for box,classname,retscore in zip(nboxes,nclasses,retscores):	
					print("for loop",box,classname,retscore )		
					ymin,xmin,ymax,xmax=int(box[0]),int(box[1]),int(box[2]),int(box[3])
					box1=[xmin,ymin,(xmax-xmin),(ymax-ymin)]  #(x,y,w,h)
					trackingage=1
					print("args to first addto_trackerlist",frame,box1,classname,retscore,trackingage)
				...
				for dbox,classname,retscore in zip(nboxes,nclasses,retscores):
					...
					if AppendData==True:
						print("classname in tracker",classname)
		...
		print("tboxesclassestrackeridsretscores", tboxes,classes,trackerids,tretscores)
		return tboxes,classes,trackerids,tretscores
